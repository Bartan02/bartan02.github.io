...
function initAetherRibbon(userConfig: Partial<AetherConfig> = {}) {
    ...
    const render = (now: number) => {
        ...
        layers.forEach((layer) => {
            ...
            gradient.addColorStop(0, `rgba(${config.colors.start}, ${layer.primary ? 0.1 : 0.02})`);
            gradient.addColorStop(0.5, `rgba(${config.colors.mid}, ${layer.primary ? 0.8 : 0.35})`);
            gradient.addColorStop(1, `rgba(${config.colors.end}, ${layer.primary ? 0.1 : 0.02})`);

            for (let r = 0; r < layer.ribbonCount; r++) {
                const ribbonProgress = layer.ribbonCount > 1 ? r / (layer.ribbonCount - 1) : 0;
                ...
                for (let x = 0; x <= width + layer.step; x += layer.step) {
                    ...
                    const wave1 = Math.sin(x * (layer.freqScale * nFreq) + time * layer.speedScale + r * 0.18) * (layer.ampScale * edgeEnvelope * nAmp);
                    const wave2 = Math.cos(x * 0.008 - time * 0.7 + r * 0.1) * (20 * config.amplitude * edgeEnvelope);
                    const wave3 = Math.sin(x * 0.018 + time * 1.4) * (8 * config.amplitude * edgeEnvelope);
                    ...
                    if (config.interactive) {
                        ...
                        mouseDisplacement = Math.sin(x * 0.015 + time * 2.6) * (mouseFactor * (layer.primary ? 50 : 25) * edgeEnvelope);
                    }

                    const y = yOffset + wave1 + wave2 + wave3 + mouseDisplacement + mouse.y * (ribbonProgress * 0.1);
